...
The engine dynamically checks for data and structural hazards during every state transition using a rigorous physical space-reservation model. It scans backward to verify data dependencies. Furthermore, strict structural latches are explicitly enforced: an instruction cannot enter any stage unless the instruction ahead of it has successfully cleared that stage. This produces perfect physical traffic jams, accumulating stalls dynamically. The fetch (\texttt{IF}) stage is also structurally aware, dynamically switching its fetch rhythm between sequential (4-stage) and safely-blocked (5-stage) to perfectly match classical academic trace rules.

\subsection{Resolution Mechanism}
\begin{itemize}
    \item \textbf{Without Forwarding:} A dependent instruction is strictly stalled \textit{before} entering the \texttt{ID} stage. It outputs \texttt{STALL} visually and remains locked until the producer instruction has completely finished its \texttt{WB} stage (for 5-stage) or \texttt{MEM} stage (for 4-stage). In a 4-stage model, due to sequential fetching, stalls will dynamically accumulate back into the \texttt{IF} stage, creating a physical logjam until data is safely written to the register file.
